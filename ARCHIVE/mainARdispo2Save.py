...
def xlsx_to_res_des(file_path:str, qr_code_col:str="Reference", lib_col:str="Designation", aimed_sheet:str="")->dict:
    # Init le dict
    ref_des_couple_dict = {}
...
def qrCode_lib_grid_pdf_gen(couple_dict:dict, file_name:list="qrCode_List", dispo:int=0)->None:
    ...
    # Output File + Name
    pdf.output(os.path.join(outPut_pdf_dir,f"{file_name}TNS9D1.pdf"))
    print(f"\n Process finished")

couple_dict = xlsx_to_res_des(file_path=file_path, aimed_sheet="Feuil1")
